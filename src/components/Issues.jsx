...
const Issues = () => {
  ...
  // Function to generate random positions within the canvas (8/12 width)
  const randomTranslate = (element) => {
    const canvas = canvasRef.current;
    const canvasWidth = canvas.offsetWidth * (8 / 12); // 8/12 width
    const canvasHeight = canvas.offsetHeight;

    const randomX = Math.random() * (canvasWidth - element.offsetWidth); // Ensure it stays within 8/12 width
  ...
  return (
    <div className='z-0 w-full h-[120vh] px-4 pt-20 bg-white rounded-b-45 font-GTWalsheimPro overflow-hidden'>
      <div className='relative flex flex-col items-center gap-12 size-full'>
        <h1 className='text-6xl text-black'>
          Tired of these <span className='bg-text bg-h'>issues</span>
        </h1>
        <div className='grid grid-cols-3 text-center px-8 py-4 rounded-xl text-18 bg-white-2 [&>*]:px-10 [&>*]:py-3 [&>*]:rounded-xl'>
          <Link to='/' className='focus:bg-purple-1'>
            Agency
          </Link>
          <Link to='/' className='focus:bg-purple-1'>
            In-house Designer
